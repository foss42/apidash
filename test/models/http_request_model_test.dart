...
void main() {
  test('Testing copyWith', () {
    ...
    final httpRequestModelcopyWith = httpRequestModel.copyWith(
  ...
  test('Testing immutability', () {
    var httpRequestModel = httpRequestModelPost10;
    var httpRequestModel2 = httpRequestModel.copyWith(
      headers: httpRequestModel.headers,
    );
    expect(httpRequestModel.headers, httpRequestModel2.headers);
    expect(
      identical(httpRequestModel.headers, httpRequestModel2.headers),
      false,
    );
  ...
  group('HttpRequestModel Auth Tests', () {
    ...
    test(
      ...
      () {
        ...
        expect(
          deserializedModel.authModel?.type,
          originalModel.authModel?.type,
        );
        expect(
          deserializedModel.authModel?.basic?.username,
          originalModel.authModel?.basic?.username,
        );
        expect(
          deserializedModel.authModel?.basic?.password,
    ...
    test('should handle copyWith for HttpRequestModel with auth', () {
      ...
      const newAuthModel = AuthModel(type: APIAuthType.bearer, bearer: newAuth);

      final copiedModel = originalModel.copyWith(authModel: newAuthModel);
    ...
    test('should handle HttpRequestModel with complex auth scenarios', () {
      ...
      expect(
        httpRequestModel.authModel?.jwt?.privateKey,
        'private-key-content',
      );
    ...
    test(
      'should handle HttpRequestModel with multiple auth types in sequence',
      () {
        const originalAuth = AuthBasicAuthModel(
          username: 'testuser',
          password: 'testpass',
