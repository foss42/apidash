...
void main() {
  ...
  test(
    'Testing contentType override by the user having no charset (#630)',
    () async {
      var responseRec = await sendHttpRequest(
  ...
  test(
    'Testing default contentType charset no longer added by dart http v1.6.0',
    () async {
      var responseRec = await sendHttpRequest(
  ...
  test('Testing latin1 charset added by user', () async {
    ...
    expect(responseData.statusCode, 200);
    expect(
      responseData.requestHeaders?['content-type'],
  ...
  test('Testing fromResponse for OPTIONS method', () async {
    ...
    expect(
      responseData.headers?['access-control-allow-methods']?.contains(
        "OPTIONS",
      ),
      true,
    );
    expect(
      responseData.headers?['allow'],
      'GET,POST,PUT,PATCH,DELETE,HEAD,OPTIONS',
    );
