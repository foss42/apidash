...
class EditMQTTTopicsState extends ConsumerState<EditMQTTTopics> {
      ...
      ref
          ...
          .update(
            mqttRequestModel: mqttModel.copyWith(
              ...
              isTopicEnabledList: isRowEnabledList.sublist(
                0,
                topicRows.length - 1,
              ),
            ),
  ...
  Widget build(BuildContext context) {
    ...
    isRowEnabledList = [...mqttModel.isTopicEnabledList];
    if (isRowEnabledList.length < subTopics.length) {
      isRowEnabledList.addAll(
        List.filled(subTopics.length - isRowEnabledList.length, true),
      );
    }
    ...
    List<DataRow> dataRows = List<DataRow>.generate(topicRows.length, (index) {
              ...
              onChanged: (value) {
                ...
                if ((requestModel?.isStreaming ?? false) &&
                  ...
                  if (oldTopic.isNotEmpty) {
                    ref
                        .read(collectionStateNotifierProvider.notifier)
                        .unsubscribeMqttTopic(selectedId!, oldTopic);
                  }
                  if (value.isNotEmpty) {
                    ref
                        .read(collectionStateNotifierProvider.notifier)
            ...
            InkWell(
              onTap: isLast
                  ...
                  : () {
                      ...
                      if ((requestModel?.isStreaming ?? false) &&
                          topicToRemove.isNotEmpty) {
                        ref
                            .read(collectionStateNotifierProvider.notifier)
                            .unsubscribeMqttTopic(selectedId!, topicToRemove);
                      }
                    },
