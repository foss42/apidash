...
class _FormDataBodyState extends ConsumerState<FormDataWidget> {
  ...
  void _onFieldChange() {
    ref
        ...
        .update(formData: formRows.sublist(0, formRows.length - 1));
  }
  ...
  Widget build(BuildContext context) {
    ...
    ref.watch(
      selectedRequestModelProvider.select(
        (value) => value?.httpRequestModel?.formData?.length,
      ),
    );
    ...
    formRows = isFormDataEmpty
        ? [kFormDataEmptyModel]
        : rF + [kFormDataEmptyModel];
    isAddingRow = false;

    List<DataColumn> columns = const [
      DataColumn2(label: Text(kNameField), size: ColumnSize.M),
      DataColumn2(label: Text('='), fixedWidth: 20),
      DataColumn2(label: Text(''), fixedWidth: 70),
      DataColumn2(label: Text(kNameValue), size: ColumnSize.L),
      DataColumn2(label: Text(''), fixedWidth: 32),
    ];

    List<DataRow> dataRows = List<DataRow>.generate(formRows.length, (index) {
          ...
          Align(
            ...
            child: Padding(
              ...
              child: ElevatedButton.icon(
                ...
                label: const Text(kLabelAddFormField, style: kTextStyleButton),
              ),
            ),
          ),
      ],
