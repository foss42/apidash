...
void main() {
  ...
  test('parse another easy cURL', () async {
    expect(
      Curl.parse(
...
--header 'host: dummyimage.com'""",
    ...
    expect(
      ...
      Curl(
        ...
        formData: [
          ...
          FormDataModel(name: "token", value: "john", type: FormDataType.text),
        ],
      ),
    );
  });
  ...
  test('tryParse success', () async {
    expect(
      Curl.tryParse('curl -X GET https://api.apidash.dev -H "test: Agent"'),
      Curl(method: 'GET', uri: apiUri, headers: {'test': 'Agent'}),
    );
  });
  ...
  baz""";
    final curl = Curl.parse(cmd);
    expect(curl.uri, Uri.parse('https://api.apidash.dev/echo'));
  });
  ...
  test('HEAD 1', () async {
    expect(
      Curl.parse(r"""curl -I 'https://api.apidash.dev'"""),
      Curl(method: 'HEAD', uri: apiUri),
    );
  });

  test('HEAD 2', () async {
    expect(
      Curl.parse(r"""curl --head 'https://api.apidash.dev'"""),
      Curl(method: 'HEAD', uri: apiUri),
    );
  });
  ...
  test('GET 1', () async {
    expect(
      Curl.parse(r"""curl --url 'https://api.apidash.dev'"""),
      Curl(method: 'GET', uri: apiUri),
    );
  });
  ...
  test('GET 3', () async {
    expect(
      Curl.parse(r"""curl 'https://api.apidash.dev/country/data?code=IND'"""),
      Curl(
        method: 'GET',
  ...
  test('GET with GitHub API', () async {
    expect(
      Curl.parse(r"""curl --url 'https://api.github.com/repos/foss42/apidash' \
  ...
  test('POST with text/plain content', () async {
    expect(
      Curl.parse(r"""curl --request POST \
