...
class ModelManager {
  static Future<AvailableModels?> fetchModelsFromRemote({
    ...
    try {
      ...
      if (resp == null) {
        log('fetchModelsFromRemote -> resp == null');
      } else {
        var remoteModels = availableModelsFromJson(resp.body);
        return remoteModels;
      }
    } catch (e) {
      log('fetchModelsFromRemote -> ${e.toString()}');
    }
    return null;
  }
  ...
  static Future<AvailableModels> fetchAvailableModels({
    ...
    try {
      ...
      log('fetchAvailableModels -> ${e.toString()}');
    }
  ...
  static Future<List<Model>?> fetchInstalledOllamaModels({
    ...
    try {
      ...
      // log("fetchInstalledOllamaModels -> $url -> ${resp?.body} -> $msg");
      if (resp == null) {
        return null;
      }
      ...
      log('fetchInstalledOllamaModels -> ${e.toString()}');
      return null;
    }
  }
}
