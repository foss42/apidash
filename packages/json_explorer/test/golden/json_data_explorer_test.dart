...
void main() {
      ...
      child: ChangeNotifierProvider(
        create: (context) {
          final store = JsonExplorerStore()
            ..buildNodes(jsonObject, areAllCollapsed: collapseAll);
  ...
  testGoldens('Interaction', (tester) async {
    Widget buildWidget({Key? key, bool collapseAll = false}) =>
        ChangeNotifierProvider(
          ...
          child: Consumer<JsonExplorerStore>(
            builder: (context, state, child) => SizedBox(
              height: 400,
              child: JsonExplorer(key: key, nodes: state.displayNodes),
            ),
          ),
        );
    ...
    final builder =
        GoldenBuilder.grid(
          ...
          ..addScenario(
            'Expand',
            buildWidget(key: expandNodeKey, collapseAll: true),
          )
          ..addScenario('Collapse node', buildWidget(key: collapseNodeKey))
          ..addScenario('Mouse hover', buildWidget(key: mouseHoverKey));
    await tester.pumpWidgetBuilder(
      builder.build(),
    ...
    await tester.tap(
      find.descendant(
        ...
        matching: find.text('prizes:', findRichText: true),
      ),
    );
    ...
    await tester.tap(
      find
          .descendant(
            ...
            matching: find.text('laureates:', findRichText: true),
          )
          .first,
    );
